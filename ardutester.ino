...
#endif
#if defined(LCD_PRINT) && defined(DEBUG_PRINT) && defined(LCD_DOGM128)
  #error Invalid Parameters: Use LCD_PRINT or DEBUG_PRINT or LCD_DOGM128
#endif
#if !defined(LCD_PRINT) && !defined(DEBUG_PRINT) && !defined(LCD_DOGM128)
  #error Invalid Parameters: Choose one of them: LCD_PRINT or DEBUG_PRINT or LCD_DOGM128
#endif
...
void CheckProbes(byte Probe1, byte Probe2, byte Probe3)
{
  ...
  #ifdef DEBUG_ 
  Serial.print("No Comp_error");
  #endif
  wdt_reset();                                  //Reset watchdog 
  #ifdef DEBUG_
  Serial.print("UpdateProbes");
  #endif
  ...
  */
  #ifdef DEBUG_
  Serial.print("CheckDepletionModeFET");
  #endif
    ...
    if (Check.Done == 0)                         //Not sure yet
    {
      ...
      if (U_1 > 3422)                            //Detected current > 4.8mA
      {
        //Distinguish PNP BJT from p-channel MOSFET
          #ifdef DEBUG_
          Serial.print("CheckBJTEnhModeMOSFET");
          #endif
...
void GetGateThreshold(byte Type)
{
  ...
  for (Counter = 0; Counter < 10; Counter++) 
  {
    ...
    if (Type & TYPE_N_CHANNEL)                   //n-channel
    {
      ...
      int t = 0;
      while (ADC_PIN & Drain_ADC) { if( ++t > 10000) break;};
    }
    else                                         //p-channel
    {
...
void CheckBJTorEnhModeMOSFET(byte BJT_Type, unsigned int U_Rl)
{
  ...
  else if ((U_Rl < 97) && (U_R_c > FET_Level))   //No BJT
  {
    ...
    */
    ...
    //Serial.print("GetGateThreshold");
    GetGateThreshold(FET_Type);
    //Serial.print("GetGateThreshold-ready");
    //Save data
    FET.G = Probes.Pin_3;
  ...
  //Serial.print("CheckBJTorEnhModeMOSFET-ready");
}
...
void DisplayValue(unsigned long Value, signed char Exponent, unsigned char Unit)
{
  ...
  if (Exponent <= 0)                             //We have to prepend "0."
  {
    ...
    lcd.lcd_data('0');
    lcd.lcd_data('.');
    if (Exponent < 0) lcd.lcd_data('0');             //Extra 0 for factor 100
  }
  ...
  {
    lcd.lcd_data(OutBuffer[Index]);                  //Display char 
    if (Index == Exponent) lcd.lcd_data('.');        //Display dot 
    Index++;                                     //Next one 
  }
...
void ShortCircuit(byte Mode)
{
  ...
  {
    lcd.lcd_clear();
    lcd.lcd_fixed_string(String);                    //Display: Remove/Create
    lcd.lcd_line(1);
    lcd.lcd_fixed_string(ShortCircuit_str);          //Display: short circuit!
    Run = 1;                                     //Enter loop
  }
...
{
  //Display info
  lcd.lcd_fixed_string(Failed1_str);                 //Display: No component
  lcd.lcd_line(1);                                   //Move to line #2
  lcd.lcd_fixed_string(Failed2_str);                 //Display: found!
...
void ShowDiode(void)
{
  ...
  */
  //Uf
  lcd.lcd_line(1);                                   //Go to line #2 
  lcd.lcd_fixed_string(Vf_str);                      //Display: Vf= 
  ...
  if (D2 == NULL)                                //Single diode 
  {
    ...
    if (I_leak > 0)                              //Show if not zero 
    {
      ...
      #endif
      #ifdef LCD_PRINT
      lcd.lcd_clear_line(1);                         //Only change line #2
      #else
      lcd.lcd_clear_line(2);
      #endif 
      lcd.lcd_fixed_string(I_R_str);                 //Display: I_R= 
  ...
  if (SkipFlag == 0)
  {
    ...
    lcd.lcd_clear_line(2);
    #endif
    lcd.lcd_fixed_string(DiodeCap_str);              //Display: C= 
...
void ShowBJT(void)
{
  ...
  if (Check.Diodes > 2)                          //Transistor is a set of two diodes :-)
  {
    ...
    if (Check.Type == TYPE_NPN)                  //NPN
    {
        ...
        lcd.component(COMPONENT_BJTN);
    }
    else                                         //PNP
    {
        String = (unsigned char *)Diode_CA_str;
        lcd.component(COMPONENT_BJTP);
    }
  ...
  lcd.lcd_testpin(BJT.C);                            //Display collector pin
  lcd.pins(BJT.B, BJT.C, BJT.E); 
  //Display hFE
  lcd.lcd_line(1);                                   //Move to line #2  
  lcd.lcd_fixed_string(hFE_str);                     //Display: h_FE= 
    ...
    if ((Diode->A == A_Pin) && (Diode->C == C_Pin))
    {
      ...
      */
      ...
      if (BJT.I_CE0 > 0)                         //Show if not zero 
      {
        ...
        lcd.lcd_clear_line(1);                       //Only change line #2
        #else
        lcd.lcd_clear_line(3);
        #endif
        lcd.lcd_fixed_string(I_CEO_str);             //Display: I_CE0= 
...
void Show_FET_IGBT_Extras(byte Symbol)
{
  ...
  lcd.lcd_fixed_string(Vth_str);                     //Display: Vth
  ...
  lcd.lcd_fixed_string(GateCap_str);                 //Display: Cgs= 
...
void ShowFET(void)
{
  ...
  if (Check.Type & TYPE_N_CHANNEL)               //n-channel
  {
    ...
    lcd.component(COMPONENT_NFET);
  }
  else                                           //p-channel
  {
    Data = 'P';
    Symbol = LCD_CHAR_DIODE1;                    // '|>|' cathode pointing to source
    lcd.component(COMPONENT_PFET);
  }
  //Display type
  if (Check.Type & TYPE_MOSFET) {                 //MOSFET
    lcd.lcd_fixed_string(MOS_str);                   //Display: MOS
    if( Data == 'P' ) {
      lcd.component(COMPONENT_PMOS);
    } else {
      lcd.component(COMPONENT_NMOS);
    }
  } else                                           //JFET
    lcd.lcd_data('J');                               //Display: J 
  lcd.lcd_fixed_string(FET_str);                     //Display: FET 
  //Display channel type
  lcd.lcd_space();
  lcd.lcd_data(Data);                                //Display: N / P
  lcd.lcd_fixed_string(Channel_str);                 //Display: -ch
  //Display mode
  if (Check.Type & TYPE_MOSFET)                  //MOSFET
  {
    lcd.lcd_space();
    if (Check.Type & TYPE_ENHANCEMENT)           //Enhancement mode 
      lcd.lcd_fixed_string(Enhancement_str);
    else                                         //Depletion mode 
    {
      lcd.lcd_fixed_string(Depletion_str);
      if( Data == 'P' ) {
        lcd.component(COMPONENT_PMOSD);
      } else {
        lcd.component(COMPONENT_NMOSD);
      }
    }
  }
  //Pins
  lcd.lcd_line(1);                                   //Move to line #2
  lcd.lcd_fixed_string(GDS_str);                     //Display: GDS=
  lcd.lcd_testpin(FET.G);                            //Display gate pin
  ...
  {
    lcd.lcd_testpin(FET.D);                          //Display drain pin
    lcd.lcd_testpin(FET.S);                          //Display source pin
  }
  lcd.pins(FET.G, FET.D, FET.S);
...
void ShowIGBT(void)
{
  ...
  if (Check.Type & TYPE_N_CHANNEL)               //n-channel
  {
    Data = 'N';
    Symbol = LCD_CHAR_DIODE2;                    // '|<|' cathode pointing to drain
    lcd.component(COMPONENT_NIGBT);
  }
  else                                           //p-channel
  {
    Data = 'P';
    Symbol = LCD_CHAR_DIODE1;                    // '|>|' cathode pointing to source
    lcd.component(COMPONENT_PIGBT);
  }
  lcd.lcd_fixed_string(IGBT_str);                    //Display: IGBT
  //Display channel type
  lcd.lcd_space();
  lcd.lcd_data(Data);                                //Display: N / P 
  lcd.lcd_fixed_string(Channel_str);                 //Display: -ch 
  ...
  //Pins
  lcd.lcd_line(1);                                   //Move to line #2 
  lcd.lcd_fixed_string(GCE_str);                     //Display: GCE= 
  lcd.lcd_testpin(FET.G);                            //Display gate pin 
  lcd.lcd_testpin(FET.D);                            //Display collector pin 
  lcd.lcd_testpin(FET.S);                            //Display emitter pin 
  lcd.pins( FET.G, FET.D, FET.S);
...
void ShowSpecial(void)
{
  ...
  {
    lcd.lcd_fixed_string(Thyristor_str);             //Display: thyristor
  }
  else if (Check.Found == COMP_TRIAC)
  {
    lcd.lcd_fixed_string(Triac_str);                 //Display: triac
  }
  //Display pins
  lcd.lcd_line(1);                                   //Move to line #2
  lcd.lcd_fixed_string(GAK_str);                     //Display: GAK 
  lcd.lcd_testpin(BJT.B);                            //Display gate pin 
  lcd.lcd_testpin(BJT.C);                            //Display anode pin 
  lcd.lcd_testpin(BJT.E);                            //Display cathode pin
  lcd.pins(BJT.B, BJT.C, BJT.E);
}
...
void ShowResistor(void)
{
  lcd.component(COMPONENT_RESISTOR);
  ...
  byte x;
  if (R1->A != Pin) { lcd.lcd_testpin(R1->A); x=R1->A;}
  else {lcd.lcd_testpin(R1->B); x=R1->B;}
  lcd.lcd_fixed_string(Resistor_str);
  lcd.lcd_testpin(Pin);
  lcd.pins(x,Pin,' ');
  if (R2)                                        //Second resistor
  {
    lcd.lcd_fixed_string(Resistor_str);
    if (R2->A != Pin) lcd.lcd_testpin(R2->A);
    else lcd.lcd_testpin(R2->B);
  }
  ...
  else                                           //Single resistor
  {
    ...
    if (MeasureInductor(R1) == 1)
    {
      ...
      lcd.component(COMPONENT_INDUCTOR);
    }
  }
}
...
void ShowCapacitor(void)
{
  lcd.component(COMPONENT_CAPACITOR);
  ...
  //Display pinout
  lcd.lcd_testpin(MaxCap->A);                        //Display pin #1
  lcd.lcd_fixed_string(Cap_str);                     //Display capacitor symbol 
  lcd.lcd_testpin(MaxCap->B);                        //Display pin #2 
  lcd.lcd_line(1);                                   //Move to line #2
  lcd.pins(MaxCap->A,MaxCap->B,' '); 
  //And show capacitance
    ...
    while (Counter <= 5)
    {
      ...
      switch (Test)
      {
        ...
        case 5:                                  //Rh resistors pulled down 
          lcd.lcd_fixed_string(RhLow_str);           //Display: Rh- 
          //TP1: Gnd -- Rh -- probe 
          R_PORT = 0;
          R_DDR = 2 << ((TP1 * 2)+R_SHIFT);
          Val1 = ReadU_20ms(TP1);
          //TP1: Gnd -- Rh -- probe 
          R_DDR = 2 << ((TP2 * 2)+R_SHIFT);
          Val2 = ReadU_20ms(TP2);
          //TP1: Gnd -- Rh -- probe 
          R_DDR = 2 << ((TP3 * 2)+R_SHIFT);
          Val3 = ReadU_20ms(TP3);
          break;
        case 6:                                  //Rh resistors pulled up 
          lcd.lcd_fixed_string(RhHigh_str);          //Display: Rh+ 
          //TP1: probe -- Rh -- Vcc 
          R_DDR = 2 << ((TP1 * 2)+R_SHIFT);
          R_PORT = 2 << ((TP1 * 2)+R_SHIFT);
          Val1 = ReadU_20ms(TP1);
          ...
          //TP1: probe -- Rh -- Vcc 
          R_DDR = 2 << ((TP3 * 2)+R_SHIFT);
          R_PORT = 2 << ((TP3 * 2)+R_SHIFT);
          Val3 = ReadU_20ms(TP3);
          break;
          #if (NUM_TESTS > 6)
         case 7:
          char comp[20][20]={"Resistor","Capacitor","Inductor","Diode","Transistor-NPN","Transistor-PNP","NFET","PFET","NIGBT","PIGBT",
           "TRIAC","Thyristor","Zener","NMOSFET","PMOSFET","BJT-NPN","BJT-PNP","NMOS-DEPLETION","PMOS-DEPLETION"};
          for( int i = 3; i < 22; i ++ ) {
            lcd.lcd_clear_line(1);
    ...
    while (Counter <= 5)
    {
      ...
      switch (Test)
      {
        case 1:                                  //Resistance of probe leads (probes shorted) 
          lcd.lcd_fixed_string(ROffset_str);         //Display: R0 
          lcd.lcd_space();
          lcd.lcd_fixed_string(ProbeComb_str);       //Display: 12 13 23           
        ...
        case 3:                                  //Internal resistance of µC in pull-down mode 
          lcd.lcd_fixed_string(RiLow_str);           //Display: Ri-
          //TP1:  Gnd -- Ri -- probe -- Rl -- Ri -- Vcc
          SetADCLow();
          ADC_DDR = 1 << TP1;
          R_PORT = 1 << ((TP1 * 2)+R_SHIFT);
          R_DDR = 1 << ((TP1 * 2)+R_SHIFT);
          Val1 = ReadU_5ms(TP1);
          U_RiL += Val1;
          //TP2: Gnd -- Ri -- probe -- Rl -- Ri -- Vcc 
          ADC_DDR = 1 << TP2;
          R_PORT =  1 << ((TP2 * 2)+R_SHIFT);
          R_DDR = 1 << ((TP2 * 2)+R_SHIFT);
          Val2 = ReadU_5ms(TP2);
          U_RiL += Val2;
          //TP3: Gnd -- Ri -- probe -- Rl -- Ri -- Vcc 
          ADC_DDR = 1 << TP3;
          R_PORT =  1 << ((TP3 * 2)+R_SHIFT);
          R_DDR = 1 << ((TP3 * 2)+R_SHIFT);
          Val3 = ReadU_5ms(TP3);
          U_RiL += Val3;
          RiL_Counter += 3;
          break;
        case 4:                                  //Internal resistance of µC in pull-up mode 
          lcd.lcd_fixed_string(RiHigh_str);          //Display: Ri+ 
        ...
        case 5:                                  //Capacitance offset (PCB and probe leads) 
          lcd.lcd_fixed_string(CapOffset_str);       //Display: C0 
          lcd.lcd_space();
          lcd.lcd_fixed_string(ProbeComb_str);       //Display: 12 13 23 
...
void ShowAdjust(void)
{
  ...
  lcd.lcd_line(5);
  #endif
  lcd.lcd_fixed_string(CompOffset_str);              //Display: AComp
  lcd.lcd_space();
...
void PWM_Tool(unsigned int Frequency)
{
  ...
  */
  ShortCircuit(0);                               //Make sure probes are not shorted
  lcd.lcd_clear();
  lcd.lcd_fixed_string(PWM_str);                     //Display: PWM
  lcd.lcd_data(' ');
  ...
  while (!Break) {
    // set Ratio value
      ...
      while (Test > 0)
      {
        ...
        #ifdef LCD_DOGM128
        lcd.component(COMPONENT_NEVER);
        lcd.lcd_clear_line(3);
        lcd.lcd_string("Info:");
        lcd.lcd_clear_line(4);
        lcd.lcd_string("Short Press +5%");
        ...
        if (Test == 1)                               //Short key press
        {
          ...
          #ifdef BUTTON_INST
            Test2 = TestKey(200, 0);             //Check for second key press
          #else
            delay(3000);
            Test2=0;
          #endif
          if (Test2 > 0)                         //Second key press
          {
            Test = 0;                                //End loop
          }
      ...
      DisplayValue(Ratio, 0, '%');                 //Show ratio in %
      lcd.lcd_line(1);
      lcd.lcd_string("Press btn to finish!");
      #ifdef LCD_DOGM128
        lcd.component(COMPONENT_NONE);
        lcd.lcd_clear_line(3);
        lcd.lcd_clear_line(4);
      ...
      #if (R_SHIFT != 0)
       // set portB0 for output and high - this connects OCR1B with Probe2 
       pinMode(8, OUTPUT); // PB0
       digitalWrite( 8, HIGH); // PB0
       pinMode(3, INPUT); // Probe 2 - set pin input mode
      #endif
      //Power save mode would disable timer1 
      Config.SleepMode = SLEEP_MODE_IDLE;            //Change sleep mode to Idle
      ICR1 = 0xffff;                                 //set timer to 16 bit mode
      ...
      //Ratio control
      Break = TestKey(0,0);                           // Wait user interrupt, short key press or whatever
      //Clean up
      #if (R_SHIFT != 0)
        // Set portB0 output and Low value
       pinMode(8, OUTPUT); // PB0
       digitalWrite( 8, LOW); // PB0
       pinMode(3, OUTPUT); // Probe 2 (PD3)
       interrupts();
      #endif
      TCCR1B = 0;                                    //Disable timer
    ...
    do
    {
      ...
      do
      {
        if (Serial.available() > 0) 
        {
          ...
          {
            case 1:                              //Pwm Menu
              #ifndef PWM_TOOL_SUPPORT
               lcd.lcd_clear();
               lcd.lcd_line(0);
               lcd.lcd_string("PWM tool is not");
               lcd.lcd_line(1);
               lcd.lcd_string("supported!");
               delay(3000);
              #else
                Serial.println();
                Frequency=selFreq();
                ...
                Serial.println(X("  Double Press Start"));
                PWM_Tool(Frequency); 
                Serial.println();
                cmdexec=true;
              #endif
...
void LcdMenu(void)
{
  ...
  if (Flag == 1)
    ...
    lcd.lcd_fixed_string(Error_str);                 //Display: error!
}
#endif // DEBUG_PRINT
...
byte MenuTool(byte Items, byte Type, void *Menu[], unsigned char *Unit)
{
  ...
  while (Run)
  {
    ...
    {
      lcd.lcd_fixed_string(*(unsigned char **)Address);
    }
    else          
    {
